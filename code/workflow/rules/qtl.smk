...
def getExtractNoisyPhenoInput(wildcards):
    if wildcards.datasource2 == 'GTEx':
        counts = os.path.join('results/pheno/noisy', wildcards.datasource2, 
        ...
        counts = os.path.join('results/pheno/noisy-geuv', wildcards.datasource2, 
                wildcards.group, 'leafcutter_perind.counts.noise.gz')
...
rule ExtractNoisyCounts:
    ...
    output:
        counts = 'results/pheno/noisy/{datasource2}/{group}/leafcutter_perind.counts.noiseonly.gz',
        indivs = 'results/pheno/noisy/{datasource2}/{group}/individuals.txt'
    run:
        import subprocess
...
rule PrepPhenoBed:
    ...
    output: 'results/pheno/noisy/{datasource2}/{group}/pheno.chr1.bed.gz'
    params:
        Rscript = 'workflow/scripts/prepPhenoBed.R',
        outprefix = 'results/pheno/noisy/{datasource2}/{group}/pheno',
...
rule PhenotypePCA:
    ...
    output: 'results/pheno/noisy/{datasource2}/{group}/{chrom}.pca'
    params:
        rscript = 'workflow/scripts/PermuteAndPCA.R',
        inputfile = 'results/pheno/noisy/{datasource2}/{group}/pheno.{chrom}.bed.gz'
    shell:
        '''
        ls {input}
...
rule ExtractGenotypeVCF:
    '''
        Procedures:
            ...
            2.  tabix vcf
    '''
    message: '### Extract genotype vcf for phenotype samples - {wildcards.datasource2}:{wildcards.group}:{wildcards.chrom}'
    input: 
        vcf = getExtractGenotypeInput,
        sample_file = rules.ExtractNoisyCounts.output.indivs
    output: 'results/geno/{datasource2}/{group}/{chrom}.vcf.gz'
    params:
...
rule GenotypePCA:
    input:  'results/geno/{datasource2}/{group}/{chrom}.vcf.gz'
    output: 'results/geno/{datasource2}/{group}/{chrom}.pca'
    params:
        out_prefix = 'results/geno/{datasource2}/{group}/{chrom}'
    shell:
        '''
            QTLtools pca \
            --seed 123 \
...
rule MakeCovarianceMatrix:
    ...
    output: 'results/pheno/noisy/{datasource2}/{group}/{chrom}_CovMatrix.txt'
    params:
        Geno_PCs = 4,
...
rule MapQTL_Perm:
    ...
    input:
        ...
        bed = 'results/pheno/noisy/{datasource2}/{group}/pheno.{chrom}.bed.gz',
        cov = 'results/pheno/noisy/{datasource2}/{group}/{chrom}_CovMatrix.txt'
    output: temp('results/qtl/noisy/{datasource2}/{group}/cis_{window}/perm/{chrom}.txt')
    params:
        cis_window = '{window}'
...
rule AddQvalueToPermutationPass:
    ...
    input:  'results/qtl/noisy/{datasource2}/{group}/cis_{window}/perm/{chrom}.txt'
    output: 'results/qtl/noisy/{datasource2}/{group}/cis_{window}/perm/{chrom}.addQval.txt.gz'
    params:
...
# rule MakePhenoBed:
#     message:'### Make QTLtools required phenotype bed file for {wildcards.datasource2}:{wildcards.group}:{wildcards.chrom}'
#     input:
#         PCs = rules.PrepPhenoTable.output
#     output:
#         bed = 'results/pheno/noisy/{datasource2}/{group}/{chrom}.bed.gz'
#     params:
#         out_dir = 'results/pheno/noisy/{datasource2}/{group}/',
#         pheno = 'results/pheno/noisy/{datasource2}/{group}/leafcutter_perind.counts.noiseonly.gz.phen_{chrom}'
#     run:
...
#     output: 'results/pheno/noisy/{datasource2}/{group}/leafcutter_perind.counts.noiseonly.gz.PCs'
#     params: 
