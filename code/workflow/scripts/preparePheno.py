...
def stream_table(f, ss = ' '):
    fc = 'chrom' # different from leafcutter1
    while fc == 'chrom':
        fc = f.readline().strip()
        head = fc.split(ss)
...
def main(ratio_file: str, sample_file: str, chroms: set, blacklist_chroms: list, pcs=50, outPrefix="out"):
    ...
    header = gzip.open(ratio_file, 'rt').readline().split()[1:] # sample names in counts file

    for i in fout:
        fout[i].write("\t".join(["#Chr","start", "end", "pid", "gid", "strand"]+header)+'\n')

    if sample_file:
        try:
            sample_names = [x.strip() for x in open(sample_file, 'r').readlines()]
            sys.stderr.write(f"Filtering using {len(sample_names)} sample names from {sample_file}...\n")
            sys.stderr.write(f"Counts file has {len(header)} samples, ")
            header = [x for x in header if x in sample_names] # filtered through sample names
            sys.stderr.write(f"keeping {len(header)} of which also in {sample_file}\n")
    ...
    for dic in stream_table(gzip.open(ratio_file, 'rt'), ' '):
        ...
        if sample_file:
            dic = {k:v for k,v in dic.items() if k in ['chrom'] + sample_names}
        ...
        gid = '.'
        if len(valRow) > 0:
            fout[chr_].write("\t".join([chr_,s,e,chrom,gid,strand]+[str(x) for x in valRow])+'\n')
            fout_ave.write(" ".join([f"{chrom}"]+[str(min(aveReads)), str(max(aveReads)), str(np.mean(aveReads))])+'\n')
    ...
    sys.stdout.write(f"Use `sh {outPrefix}_prepare.sh' to create index for fastQTL (requires tabix and bgzip).\n")
...
if __name__ == "__main__":
    ...
    parser.add_option("--outPrefix", dest="outPrefix", default="leafcutter", help="prefix for output files")
    parser.add_option("--sampleFile", dest="sampleFile", default=None, help="List of sample names to filter counts file by. If none is provided, will use all samples in counts file")
    (options, args) = parser.parse_args()
    if len(args)==0:
        sys.stderr.write("Error: no ratio file provided...\n")
        exit(0)
    ratio_file, npcs, chrom_blacklist, outPrefix, sampleFile = args[0], int(options.npcs), options.cbl, options.outPrefix, options.sampleFile
    chrom_list, chrom_blacklist = get_chromosomes(ratio_file), get_blacklist_chromosomes(chrom_blacklist)
