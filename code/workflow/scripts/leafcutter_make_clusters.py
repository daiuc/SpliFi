...
def pool_junc_reads(flist, options):
    ...
    outFile = f"{rundir}/{outPrefix}_pooled_mkclu"
...
def refine_clusters(options):
    ...
    inFile = f"{rundir}/{outPrefix}_pooled_mkclu"
    outFile = f"{rundir}/{outPrefix}_clusters"
    fout = open(outFile,'w')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--minclureads", dest="minclureads", default = 0,
        help="minimum reads in a cluster (default 0 reads)")

    parser.add_argument("-M", "--minreads", dest="minreads", default = 0,
                  help="minimum reads for a junction to be considered for \
                        clustering. (default 0 reads)")
